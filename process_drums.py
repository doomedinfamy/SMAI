...
process_drums.py
================
Apply the *drum* correction values from `corrected_songs.csv` to an actual
drum track, using four classic mixing techniques:

    1. Aggressive Noise Gating   - clamp everything below a threshold to
                                   silence with fast attack / short hold /
                                   fast release. Tightens sloppy drums and
                                   kills bleed/room noise.
    2. Transient Shaping         - split the signal into an "attack" and a
                                   "sustain" envelope and re-balance them,
                                   so you can add punch (boost attack) or
                                   dry it out (cut sustain).
    3. Volume Shaper / LFO Ducking - a tempo-synced gain LFO that pumps the
                                   level rhythmically (the classic sidechain
                                   "duck" on each beat).
    4. Correction application    - the target gain_drums / eq_drums_* /
                                   comp_drums / pan_drums from the corrections
                                   file are dialed in (level, 3-band EQ,
                                   compression, pan).

Input can be an isolated drum stem, or a full song (drums are separated with
demucs if available, else a percussive-component approximation).

The correction row is read from `corrected_songs.csv` (produced by
`apply_corrections.py`) by --song-id, or you can pass explicit values.

Dependencies
------------
    pip install numpy scipy soundfile librosa pandas

Usage
-----
    python process_drums.py drums.wav
    python process_drums.py song.wav --from-song --song-id 0
    python process_drums.py drums.wav --corrections corrected_songs.csv --song-id 3
    python process_drums.py drums.wav --tempo 128 --gate-db -40 --duck-depth 0.6
"""
